          ...
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form @submit.prevent="submit" class="space-y-4 md:space-y-6" action="#">
                  ...
                  <div class="flex items-center justify-between">
                      ...
                      <a href="#" class="text-sm font-medium text-primary-600 hover:underline dark:text-white">Mot de passe oublié?</a>
                  </div>
                  <button type="submit" class="w-full text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Connexion</button>
...
export default {
  ...
  setup() {
    ...
    const submit = async () => {
    try {
        const response = await axios.post('http://localhost:8000/api/user/auth/jwt/create/', data, {
            withCredentials: true
        });


        // Enregistrement des tokens d'authentification
        ...
        // Définition de l'en-tête d'autorisation pour les futures requêtes
